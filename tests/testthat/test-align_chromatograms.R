...
set.seed(1)
out2 <- align_chromatograms(data, sep = "\t", rt_col_name = "time", write_output = NULL, rt_cutoff_low = 15, rt_cutoff_high = 30, reference = NULL,max_linear_shift = 0.05, max_diff_peak2mean = 0.02, min_diff_peak2peak = 0.03, blanks = NULL,delete_single_peak = TRUE)

test_that("output is correct", {
    expect_equal(out[["Logfile"]][["Aligned"]][["retained"]], 115) # Number of Peaks
    ...
    expect_equal(out2[["Logfile"]][["Variation"]][["Aligned"]][["Max"]], 0.01) # Variation of Rts
